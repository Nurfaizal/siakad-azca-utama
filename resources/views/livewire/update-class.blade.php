<div>
    ...
    <form class="my-6" action="/kelas/{{ Crypt::encrypt($classes->id_class) }}" method="post">
        @csrf
        @method('put')
        <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
            <div>
                <label for="name" class="required mb-2 block text-sm font-medium text-gray-600">Nama
                    Kelas</label>
                <input type="text" id="name" name="name" class="@error('name') border-red-600 @enderror block w-full rounded-lg border bg-gray-50 p-2.5 text-sm text-gray-600 focus:border-red-500 focus:ring-red-500" placeholder="Kelas..." value="{{ $classes->name }}" />
                @error('name')
            ...
            <div class="grid grid-cols-2 gap-3">
                <div class="relative">
                    <label for="time_in" class="required mb-2 block text-sm font-medium text-gray-600">Jam
                        Masuk</label>
                    <input type="time" id="time_in" name="time_in" class="@error('time_in') border-red-600 @enderror w-full rounded-lg border bg-gray-50 p-2.5 text-sm text-gray-600 focus:border-red-500 focus:ring-red-500" value="{{ $classes->time_in }}" />
                ...
                <div class="relative">
                    <label for="time_out" class="required mb-2 block text-sm font-medium text-gray-600">Jam
                        Keluar</label>
                    <input type="time" id="time_out" name="time_out" class="@error('time_out') border-red-600 @enderror w-full rounded-lg border bg-gray-50 p-2.5 text-sm text-gray-600 focus:border-red-500 focus:ring-red-500" value="{{ $classes->time_out }}" />
                    <i class="bi bi-alarm absolute right-4 top-1/2"></i>
                    @error('time_out')
                        <p class="pt-1 text-xs text-red-600">{{ $message }}</p>
                    @enderror
            ...
            <div>
                ...
                <select name="id_staff" id="id_staff" name="id_staff" class="@error('id_staff') border-red-600 @enderror js-select2 block w-full rounded-lg border bg-gray-50 p-2.5 text-sm text-gray-600 focus:border-red-500 focus:ring-red-500">
                    <option selected value="{{ $classes->id_staff }}">-- {{ $classes->staff->name }} --</option>
                    @foreach ($staff as $s)
            ...
            <div>
                ...
                <select name="status" id="status" name="status" class="@error('status') border-red-600 @enderror block w-full rounded-lg border bg-gray-50 p-2.5 text-sm text-gray-600 focus:border-red-500 focus:ring-red-500">
                    <option selected value="{{ $classes->status }}">-- {{ $classes->status }} --</option>
        ...
        <div class="mt-10 flex justify-end">
            <button type="submit" {{ count($staff) < 1 ? 'disabled' : '' }} class="dark:bg-red-600 dark:hover:bg-red-700 dark:focus:ring-red-800 w-full rounded-lg bg-red-700 px-5 py-2.5 text-center text-sm font-medium text-white hover:bg-red-800 focus:outline-none focus:ring-4 focus:ring-red-300 disabled:bg-red-300 sm:w-auto">Simpan</button>
        </div>
    </form>
</div>
