...
@section('content')
    <div class="grid grid-cols-1 rounded-lg bg-white px-5 py-6">
        <div class="header flex flex-col items-center justify-between gap-5 border-b pb-5 md:flex-row">
            <div>
                <h1 class="text-lg font-semibold text-red-600"><i class="bi bi-book-half pe-2"></i> Hapus Semester</h1>
            </div>
            <div>
        ...
        <div class="body">
            <form class="my-6" action="/jenis-semester/{{ Crypt::encrypt($semester_type->id_semester_type) }}" method="post">
                ...
                <table>
                    <tr height="40">
                        <td width="150"><b>Nama Jenis Semester</b></td>
                        <td width="12">:</td>
                        <td>{{ $semester_type->name }}</td>
                    </tr>
                    <tr height="40">
                        <td><b>Status</b></td>
                        <td>:</td>
                        <td>
                            @if ($semester_type->status == 'Aktif')
                                <span class="me-2 rounded-sm bg-green-100 px-2.5 py-0.5 text-sm font-medium text-green-800">{{ $semester_type->status }}</span>
                            @else
                                <span class="me-2 rounded-sm bg-indigo-100 px-2.5 py-0.5 text-sm font-medium text-indigo-800">{{ $semester_type->status }}</span>
                            @endif
                        </td>
                    </tr>
                </table>
