...
@section('content')
    <div class="grid grid-cols-1 rounded-lg bg-white px-5 py-6">
        <div class="header flex flex-col items-center justify-between gap-5 border-b pb-5 md:flex-row">
            <div>
                <h1 class="text-lg font-semibold text-red-600"><i class="bi bi-book-half pe-2"></i> Daftar Semester</h1>
            </div>
            <div>
            ...
            <table id="pagination-table" class="w-full">
                <thead>
                    <tr>
                        ...
                        <th class="border">
                            <div class="flex h-full items-center">Jenis Semester</div>
                        </th>
                        <th class="border">
                ...
                <tbody>
                    @foreach ($semester as $s)
                        <tr>
                            ...
                            @if ($s->final_level != 0)
                                <td>{{ $s->name }} - Tingkat Akhir</td>
                            @else
                                <td>{{ $s->name }}</td>
                            @endif
                            <td>{{ $s->code }}</td>
                            <td>{{ $s->semester_type->name }}</td>
                            <td>
                                @if ($s->status == 'Aktif')
                            ...
                            <td class="text-center">
                                <a href="/semester/{{ Crypt::encrypt($s->id_semester) }}"><i class="bi bi-eye mr-3 text-lg text-green-600 hover:text-green-900"></i></a>
                                <a href="/semester/{{ Crypt::encrypt($s->id_semester) }}/edit"><i class="bi bi-pencil-square mr-3 text-lg text-green-600 hover:text-green-900"></i></a>
                                <a href="/semester/{{ Crypt::encrypt($s->id_semester) }}/delete"><i class="bi bi-trash text-lg text-green-600 hover:text-green-900"></i></a>
                            </td>
                        </tr>
                    @endforeach
                </tbody>
