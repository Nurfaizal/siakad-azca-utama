...
@section('content')
    <div class="grid grid-cols-1 rounded-lg bg-white px-5 py-6">
        <div class="header flex flex-col items-center justify-between gap-5 border-b pb-5 md:flex-row">
            <div>
                <h1 class="text-lg font-semibold text-red-600"><i class="bi bi-book-half pe-2"></i> Ubah Mata Pelajaran</h1>
            </div>
            <div>
        ...
        <div class="body">
            <form class="my-6" action="/mapel/{{ Crypt::encrypt($subject->id_subject) }}" method="post">
                @csrf
                @method('PUT')
                <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
                    <div>
                        <label for="name" class="mb-2 block text-sm font-medium text-gray-600">Nama Mata
                            Pelajaran</label>
                        <input type="text" id="name" name="name" class="@error('name') border-red-600 @enderror block w-full rounded-lg border p-2.5 text-sm text-gray-600 focus:border-red-500 focus:ring-red-500" placeholder="Nama Mata Pelajaran..." value="{{ $subject->name }}" autocomplete="off" />
                        @error('name')
                    ...
                    <div>
                        <label for="subject_code" class="mb-2 block text-sm font-medium text-gray-600">Kode Mata
                            Pelajaran</label>
                        <input type="text" id="subject_code" name="subject_code" class="@error('subject_code') border-red-600 @enderror block w-full rounded-lg border p-2.5 text-sm text-gray-600 focus:border-red-500 focus:ring-red-500" placeholder="Kode Mata Pelajaran..." value="{{ $subject->subject_code }}" autocomplete="off" />
                        @error('subject_code')
                            <p class="pt-1 text-xs text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                    <div>
                        ...
                        <select name="id_subcontent" id="id_subcontent" name="id_subcontent" class="@error('id_subcontent') border-red-600 @enderror js-select2 block w-full rounded-lg border p-2.5 text-sm text-gray-600 focus:border-red-500 focus:ring-red-500">
                            <option selected value="{{ $subject->id_subcontent }}">-- {{ $subject->subject_content->name }}
                                --</option>
                    ...
                    <div>
                        ...
                        <select name="level" id="level" name="level" class="@error('level') border-red-600 @enderror js-select2 block w-full rounded-lg border bg-gray-50 p-2.5 text-sm text-gray-600 focus:border-red-500 focus:ring-red-500">
                            <option value="{{ $subject->level }}">-- {{ $subject->level }} --</option>
                    ...
                    <div>
                        ...
                        <select name="status" id="status" name="status" class="@error('status') border-red-600 @enderror block w-full rounded-lg border p-2.5 text-sm text-gray-600 focus:border-red-500 focus:ring-red-500">
                            <option selected value="{{ $subject->status }}">-- {{ $subject->status }} --</option>
