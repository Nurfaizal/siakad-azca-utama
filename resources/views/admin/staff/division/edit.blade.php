...
@section('content')
    <div class="grid grid-cols-1 rounded-lg bg-white px-5 py-6">
        <div class="header flex flex-col items-center justify-between gap-5 border-b pb-5 md:flex-row">
            <div>
                <h1 class="text-lg font-semibold text-red-600"><i class="bi bi-book-half pe-2"></i> Edit Divisi</h1>
            </div>
            <div>
        ...
        <div class="body">
            ...
            <form class="my-6" action="/divisi/{{ Crypt::encrypt($division->id_division) }}" method="post">
                @csrf
                @method('PUT')

                <div>
                    <label for="name" class="mb-2 block text-sm font-medium text-gray-600">Nama Divisi</label>
                    <input type="text" id="name" name="name" class="@error('name') border-red-600 @enderror block w-full rounded-lg border bg-gray-50 p-2.5 text-sm text-gray-600 focus:border-red-500 focus:ring-red-500" placeholder="Nama Divisi..." value="{{ $division->name }}" autocomplete="off" />
                    @error('name')
                ...
                <div class="mt-4 grid grid-cols-2 gap-3">
                    <div class="relative">
                        ...
                        <input type="time" id="time_in" name="time_in" class="@error('time_in') border-red-600 @enderror w-full rounded-lg border p-2.5 text-sm text-gray-600 focus:border-red-500 focus:ring-red-500" value="{{ $division->time_in }}" />
                    ...
                    <div class="relative">
                        ...
                        <input type="time" id="time_out" name="time_out" class="@error('time_out') border-red-600 @enderror w-full rounded-lg border p-2.5 text-sm text-gray-600 focus:border-red-500 focus:ring-red-500" value="{{ $division->time_out }}" />
                        <i class="bi bi-alarm absolute right-4 top-1/2"></i>
                        @error('time_out')
                            <p class="pt-1 text-xs text-red-600">{{ $message }}</p>
                        @enderror
