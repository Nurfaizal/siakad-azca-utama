...
@section('content')
    <div class="grid grid-cols-1 px-5 py-6 bg-white rounded-lg">
        <div class="flex flex-col items-center justify-between gap-5 pb-5 border-b header md:flex-row">
            <div>
                <h1 class="text-lg font-semibold text-red-600"><i class="bi bi-book-half pe-2"></i> Ubah Gaji Guru & Staff
                </h1>
            </div>
            <div>
                <a href="/gaji-staff"
        ...
        <div class="body">
            <form class="my-6" action="/gaji-staff/{{ Crypt::encrypt($staff->salaries->id) }}" enctype="multipart/form-data"
                method="post">
                @method('put')
                @csrf
                <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
                    <div>
                                ...
                                @foreach ($salaryDeduction as $sd)
                                    <div class="grid grid-cols-2 gap-2">
                                        <input type="text" id="salary_deduction_description"
                                            ...
                                            placeholder="Tipe" value="{{ $sd->salary_deduction_description }}" />
                                        <input type="text" id="salary_deduction" name="salary_deduction[]"
                                            class="block money w-full rounded-lg border p-2.5 text-sm text-gray-600 focus:border-red-500 focus:ring-red-500"
                                            placeholder="Nilai" value="{{ $sd->salary_deduction }}" />
                                    </div>
                                @endforeach
                            </div>
                        ...
                        <div class="mb-3">
                            <div id="salary-bonus-section">
                                ...
                                @foreach ($salaryBonus as $sb)
                                    <div class="grid grid-cols-2 gap-2">
                                        <input type="text" id="salary_bonus_description"
                                            name="salary_bonus_description[]"
                                            ...
                                            placeholder="Tipe" value="{{ $sb->salary_bonus_description }}" />
                                        <input type="text" id="salary_bonus" name="salary_bonus[]"
                                            ...
                                            placeholder="Nilai" value="{{ $sb->salary_bonus }}" />
                                    </div>
                                @endforeach
                            </div>
                    ...
                    <div>
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" id="salary_deduction_total" name="salary_deduction_total"
                                class=" @error('salary_deduction_total') border-red-600 @enderror block w-full rounded-lg border p-2.5 text-sm text-gray-600 focus:border-red-500 focus:ring-red-500"
                                value="{{ $staff->salaries->salary_deduction_total }}" />
                        ...
                        <div class="mb-3">
                            ...
                            <input type="number" id="salary_bonus_total" name="salary_bonus_total"
                                class=" @error('salary_bonus_total') border-red-600 @enderror block w-full rounded-lg border p-2.5 text-sm text-gray-600 focus:border-red-500 focus:ring-red-500"
                                value="{{ $staff->salaries->salary_bonus_total }}" />
                        ...
                        <div class="mb-3">
                            ...
                            <input type="number" id="paid" name="paid"
                                class="@error('paid') border-red-600 @enderror block w-full rounded-lg border p-2.5 text-sm text-gray-600 focus:border-red-500 focus:ring-red-500"
                                value="{{ $staff->salaries->paid }}" />
                            @error('paid')
                                <p class="pt-1 text-xs text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
                        <div class="mb-3">
                            ...
                            <select name="month" id="month" disabled
                                class="@error('month') border-red-600 @enderror disabled:bg-gray-200 block w-full rounded-lg border p-2.5 text-sm text-gray-600 focus:border-red-500 focus:ring-red-500">
            ...
            12 => 'Desember',
                        ...
                        <div class="mb-5">
                            ...
                            <input type="number" id="year" name="year" disabled
                                class="@error('year') border-red-600 @enderror disabled:bg-gray-200 block w-full rounded-lg border p-2.5 text-sm text-gray-600 focus:border-red-500 focus:ring-red-500"
                                value="{{ $currentYear }}" />
                            @error('year')
                                <p class="pt-1 text-xs text-red-600">{{ $message }}</p>
                            @enderror
                        ...
                        <div class="flex gap-3 mb-3">
                            ...
                            <input type="radio" id="payment_method3" name="payment_method"
                                class="@error('payment_method') border-red-600 @enderror  border p-2.5 text-sm text-gray-600 focus:border-red-500 focus:ring-red-500"
                                value="debit" {{ $staff->salaries->payment_method == 'debit' ? 'checked' : '' }} />
                        ...
                        <div class="mb-5">
                            ...
                            <textarea id="description" name="description"
                                class="@error('description') border-red-600 @enderror block w-full rounded-lg border p-2.5 text-sm text-gray-600 focus:border-red-500 focus:ring-red-500">{{ $staff->salaries->description }}</textarea>
                            @error('description')
                                <p class="pt-1 text-xs text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
    ...
    <script>
        ...
        function calculate() {
            ...
            let totalIncome = parseInt(`{{ $staff->salary }}`) - parseInt($("#salary_deduction_total").val()) +
                parseInt($("#salary_bonus_total").val())
            // Hitung pajak
            let taxAmount = (tax !== 0) ? (totalIncome * tax) / 100 : 0;
        ...
        $('#tax').on('keyup', function() {
            ...
            let finalAmount = totalIncome - taxAmount;
            $("#paid").val(finalAmount);
        });
