...
@section('content')
    <div class="grid grid-cols-1 rounded-lg bg-white px-5 py-6">
        <div class="header flex flex-col items-center justify-between gap-5 border-b pb-5 md:flex-row">
            <div>
                <h1 class="text-lg font-semibold text-red-600"><i class="bi bi-book-half pe-2"></i> Hapus Catatan Siswa</h1>
            </div>
            <div>
        ...
        <div class="body">
            <form class="my-6" action="/catatan-siswa/{{ Crypt::encrypt($student_note->id_student_note) }}" method="post">
                @csrf
                @method('DELETE')
