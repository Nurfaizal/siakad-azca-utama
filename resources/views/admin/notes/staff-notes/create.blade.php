...
@section('content')
    <div class="grid grid-cols-1 rounded-lg bg-white px-5 py-6">
        <div class="header flex flex-col items-center justify-between gap-5 border-b pb-5 md:flex-row">
            <div>
                <h1 class="text-lg font-semibold text-red-600"><i class="bi bi-book-half pe-2"></i> Tambah Catatan Guru &
                    Staff
                </h1>
            </div>
            <div>
        ...
        <div class="body">
            <form class="my-6" action="/catatan-staff" enctype="multipart/form-data" method="post">
                @csrf
                ...
                <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
                    <div>
                        ...
                        <select name="id_staff" id="id_staff" name="id_staff" class="@error('id_staff') border-red-600 @enderror js-select2 block w-full rounded-lg border bg-gray-50 p-2.5 text-sm text-gray-600 focus:border-red-500 focus:ring-red-500">
                            <option selected value="">-- Pilih Guru & Staff --</option>
                            @foreach ($staff as $s)
                    ...
                    <div>
                        ...
                        <textarea name="note" id="note" rows="2" class="@error('note') border-red-600 @enderror mt-2 w-full rounded-lg border p-2">{{ old('note') }}</textarea>
                        @error('note')
                            <p class="pt-1 text-xs text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
