...
@section('content')
    <div class="grid grid-cols-1 rounded-lg bg-white px-5 py-6">
        <div class="header flex flex-col items-center justify-between gap-5 border-b pb-5 md:flex-row">
            <div>
                <h1 class="text-lg font-semibold text-red-600"><i class="bi bi-book-half pe-2"></i> Hapus Kategori Ujian</h1>
            </div>
            <div>
            ...
            <form class="my-6" action="/kategori-ujian/{{ Crypt::encrypt($exam_category->id_exam_category) }}" method="post">
                ...
                <table>
                    ...
                    <tr height="40">
                        ...
                        <td>:</td>
                        <td>
                            @if ($exam_category->status == 'Aktif')
                                <span class="me-2 rounded-sm bg-green-100 px-2.5 py-0.5 text-sm font-medium text-green-800">{{ $exam_category->status }}</span>
                            @else
                                <span class="me-2 rounded-sm bg-indigo-100 px-2.5 py-0.5 text-sm font-medium text-indigo-800">{{ $exam_category->status }}</span>
                            @endif
                        </td>
                    </tr>
                </table>
