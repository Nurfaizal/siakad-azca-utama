...
@section('content')
    <div class="grid grid-cols-1 rounded-lg bg-white px-5 py-6">
        ...
        <div class="header flex flex-col items-center justify-between gap-5 border-b pb-5 md:flex-row">
            <div>
                <h1 class="text-lg font-semibold text-red-600"><i class="bi bi-book-half pe-2"></i> E-Dokumen Siswa
                </h1>
            </div>
            <div class="flex">
                ...
                <tbody>
                    @foreach ($document as $d)
                        <tr>
                            ...
                            <td>{{ $d->student->classes->name }}</td>
                            <td>{{ $d->document_category->name }}</td>
                            <td class="flex justify-center gap-2">
                                <a href="/e-dokumen-siswa/{{ Crypt::encrypt($d->id_e_document_student) }}"><i class="bi bi-eye mr-3 text-lg text-green-600 hover:text-green-900"></i></a>
                                <a href="/e-dokumen-siswa/{{ Crypt::encrypt($d->id_e_document_student) }}/edit"><i class="bi bi-pencil-square mr-3 text-lg text-green-600 hover:text-green-900"></i></a>
                                <a href="/e-dokumen-siswa/{{ Crypt::encrypt($d->id_e_document_student) }}/delete"><i class="bi bi-trash text-lg text-green-600 hover:text-green-900"></i></a>
                            </td>
                        </tr>
                    @endforeach
