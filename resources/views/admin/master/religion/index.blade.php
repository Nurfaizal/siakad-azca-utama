...
@section('content')
    <div class="grid grid-cols-1 rounded-lg bg-white px-5 py-6">
        <div class="header flex flex-col items-center justify-between gap-5 border-b pb-5 md:flex-row">
            <div>
                <h1 class="text-lg font-semibold text-red-600"><i class="bi bi-book-half pe-2"></i> Daftar Data Agama</h1>
            </div>
            <div>
                <a href="/agama/create">
            ...
            <table id="pagination-table" class="w-full">
                <thead>
                    <tr>
                        ...
                        <th class="border">
                            <div class="flex h-full items-center">Nama Agama</div>
                        </th>
                        <th class="border">
                ...
                <tbody>
                    @foreach ($religion as $r)
                        <tr>
                            ...
                            <td>
                                @if ($r->status == 'Aktif')
                                    <span class="me-2 rounded-sm bg-green-100 px-2.5 py-0.5 text-sm font-medium text-green-800">{{ $r->status }}</span>
                                @else
                                    <span class="me-2 rounded-sm bg-indigo-100 px-2.5 py-0.5 text-sm font-medium text-indigo-800">{{ $r->status }}</span>
                                @endif
                            </td>
                            <td class="text-center">
                                <a href="/agama/{{ Crypt::encrypt($r->id_religion) }}/edit"><i class="bi bi-pencil-square mr-3 text-lg text-green-600 hover:text-green-900"></i></a>
                                <a href="/agama/{{ Crypt::encrypt($r->id_religion) }}/delete"><i class="bi bi-trash text-lg text-green-600 hover:text-green-900"></i></a>
                            </td>
                        </tr>
                    @endforeach
                </tbody>
