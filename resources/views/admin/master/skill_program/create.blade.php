...
@section('content')
    <div class="grid grid-cols-1 rounded-lg bg-white px-5 py-6">
        <div class="header flex flex-col items-center justify-between gap-5 border-b pb-5 md:flex-row">
            <div>
                <h1 class="text-lg font-semibold text-red-600"><i class="bi bi-book-half pe-2"></i> Tambah Program Keahlian</h1>
            </div>
            <div>
        ...
        <div class="body">
            ...
            <form class="my-6" action="/program-keahlian" method="post">
                @csrf

                <div>
                    <label for="name" class="mb-2 block text-sm font-medium text-gray-600">Program Keahlian</label>
                    <input type="text" id="name" name="name" class="@error('name') border-red-600 @enderror block w-full rounded-lg border bg-gray-50 p-2.5 text-sm text-gray-600 focus:border-red-500 focus:ring-red-500" placeholder="Nama Program Keahlian..." value="{{ old('name') }}" autocomplete="off" />
                    @error('name')
