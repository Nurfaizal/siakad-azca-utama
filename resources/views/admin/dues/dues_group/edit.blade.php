...
@section('content')
    <div class="grid grid-cols-1 rounded-lg bg-white px-5 py-6">
        <div class="header flex flex-col items-center justify-between gap-5 border-b pb-5 md:flex-row">
            <div>
                <h1 class="text-lg font-semibold text-red-600"><i class="bi bi-book-half pe-2"></i> Edit Grup Iuran</h1>
            </div>
            <div>
        ...
        <div class="body">
            ...
            <form class="my-6" action="/grup-iuran/{{ Crypt::encrypt($dues_group->id_dues_group) }}" method="post">
                @csrf
                @method('PUT')
                ...
                <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
                    <div>
                        ...
                        <input type="text" id="name" name="name" class="@error('name') border-red-600 @enderror block w-full rounded-lg border bg-gray-50 p-2.5 text-sm text-gray-600 focus:border-red-500 focus:ring-red-500" placeholder="Nama..." value="{{ $dues_group->name }}" autocomplete="off" />
                        @error('name')
                    ...
                    <div>
                        ...
                        <select name="type" id="type" name="type" class="@error('type') border-red-600 @enderror block w-full rounded-lg border bg-gray-50 p-2.5 text-sm text-gray-600 focus:border-red-500 focus:ring-red-500">
                            <option selected value="{{ $dues_group->type }}">-- {{ $dues_group->type }} --</option>
                    ...
                    <div>
                        ...
                        <input type="text" id="amount" name="amount" class="@error('amount') border-red-600 @enderror money block w-full rounded-lg border bg-gray-50 p-2.5 text-sm text-gray-600 focus:border-red-500 focus:ring-red-500" placeholder="Nominal..." value="{{ $dues_group->amount }}" autocomplete="off" />
                        @error('amount')
                            <p class="pt-1 text-xs text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <input type="date" id="due_date" name="due_date" class="@error('due_date') border-red-600 @enderror block w-full rounded-lg border bg-gray-50 p-2.5 text-sm text-gray-600 focus:border-red-500 focus:ring-red-500" value="{{ $dues_group->due_date }}" autocomplete="off" />
                        @error('due_date')
                            <p class="pt-1 text-xs text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <textarea id="description" name="description" rows="3" class="block w-full rounded-lg border border-gray-300 bg-gray-50 p-2.5 text-sm text-gray-900 focus:border-blue-500 focus:ring-blue-500" placeholder="Deskripsi...">{{ $dues_group->description }}</textarea>
                        @error('description')
                            <p class="pt-1 text-xs text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <select name="status" id="status" name="status" class="@error('status') border-red-600 @enderror block w-full rounded-lg border bg-gray-50 p-2.5 text-sm text-gray-600 focus:border-red-500 focus:ring-red-500">
                            <option selected value="{{ $dues_group->status }}">-- {{ $dues_group->status }} --</option>
