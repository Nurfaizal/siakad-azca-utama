...
@section('content')
    <div class="grid grid-cols-1 rounded-lg bg-white px-5 py-6">
        <div class="header flex flex-col items-center justify-between gap-5 border-b pb-5 md:flex-row">
            <div>
                <h1 class="text-lg font-semibold text-red-600"><i class="bi bi-book-half pe-2"></i> Edit Berita/Pengumuman
                </h1>
            </div>
            <div>
        ...
        <div class="body">
            ...
            <form class="my-6" action="/berita-pengumuman/{{ Crypt::encrypt($news->id_news) }}" method="post" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                ...
                <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
                    <div>
                        <div>
                            ...
                            <input type="text" id="title" name="title" class="@error('title') border-red-600 @enderror block w-full rounded-lg border bg-gray-50 p-2.5 text-sm text-gray-600 focus:border-red-500 focus:ring-red-500" placeholder="Judul..." value="{{ $news->title }}" autocomplete="off" />
                            @error('title')
                        ...
                        <div>
                            ...
                            <select name="status" id="status" name="status" class="@error('status') border-red-600 @enderror block w-full rounded-lg border bg-gray-50 p-2.5 text-sm text-gray-600 focus:border-red-500 focus:ring-red-500">
                                <option selected value="{{ $news->status }}">-- {{ $news->status }} --</option>
                        ...
                        <div>
                            ...
                            <select name="type" id="type" name="type" class="@error('type') border-red-600 @enderror block w-full rounded-lg border bg-gray-50 p-2.5 text-sm text-gray-600 focus:border-red-500 focus:ring-red-500">
                                <option selected value="{{ $news->type }}">-- {{ $news->type == 'Berita' ? '📢' : '📝' }} {{ $news->type }} --</option>
                        ...
                        <div>
                            ...
                            <input type="text" id="short_desc" name="short_desc" class="@error('short_desc') border-red-600 @enderror block w-full rounded-lg border bg-gray-50 p-2.5 text-sm text-gray-600 focus:border-red-500 focus:ring-red-500" placeholder="Keterangan..." value="{{ $news->short_desc }}" autocomplete="off" />
                            @error('short_desc')
                                <p class="pt-1 text-xs text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
                    ...
                    <div>
                        ...
                        <div>
                            ...
                            <img src="data:image/{{ $fileExt }};base64,{{ $fileData }}" alt="Google Drive Image" class="h-36 w-36 rounded object-cover">
