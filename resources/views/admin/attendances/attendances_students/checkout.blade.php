...
@section('content')
    <div class="grid grid-cols-1 rounded-lg bg-white px-5 py-6">
        ...
        <div class="header flex flex-col items-center justify-between gap-5 border-b pb-5 md:flex-row">
            ...
            <div class="flex">
                <a href="/absensi-siswa/{{ Crypt::encrypt($attendance->id_schedule) }}"
        ...
        <div class="body mt-5">
            @if ($errors->any())
                <div class="bg-red-100 border my-3 border-red-400 text-red-700 px-4 py-3 rounded mb-4">
            ...
            <form action="/absensi-siswa/{{ Crypt::encrypt($attendance->id_attendance_student) }}/checkout" method="post"
                    ...
                    <table class="tables w-full mb-10">
                        <tbody>
                            <tr>
                                ...
                                <td class="text-slate-700 font-light">{{ $attendance->schedule->subject->name }}</td>
                            </tr>
                            <tr>
                                <td class="text-slate-700 font-light">Kelas</td>
                                <td class="text-slate-700 font-light">:</td>
                                <td class="text-slate-700 font-light">{{ $attendance->schedule->class->name }}</td>
                            </tr>
                            <tr>
                                <td class="text-slate-700 font-light">Check In</td>
                                <td class="text-slate-700 font-light">:</td>
                                <td class="text-slate-700 font-light">{{ $attendance->check_in }}</td>
                            </tr>
                            <tr>
                                <td class="text-slate-700 font-light">Check Out</td>
                                <td class="text-slate-700 font-light">:</td>
                                <td class="text-slate-700 font-light" id="check_out_time"></td>
                            </tr>
                            <tr>
                                <td class="text-slate-700 font-light">Tanggal</td>
                                <td class="text-slate-700 font-light">:</td>
                                <td class="text-slate-700 font-light">{{ $attendance->date }}
                                </td>
                            </tr>
                            <tr>
                                ...
                                <td class="text-slate-700 font-light">
                                    <select name="mode" id="mode" name="mode"
                                        ...
                                        <option selected value="{{ $attendance->mode }}">--
                                            {{ $attendance->mode == 'offline' ? 'Luring' : 'Daring' }}
                            ...
                            <tr>
                                ...
                                <td class="text-slate-700 font-light">
                                    @if ($attendance->image != null)
                                        <img src="{{ $attendance->image != null ? 'data:image/' . $fileExt . ';base64,' . $fileData : '' }}"
                                            alt="Foto Staff" class="object-cover w-64 h-40 my-2">
                                    @endif
                            ...
                            <tr>
                                ...
                                <td class="text-slate-700 font-light">
                                    <textarea name="description" id="description" rows="3"
                                        class="@error('description') border-red-600 @enderror mt-2 w-full rounded-lg border p-2" placeholder="Deskripsi...">{{ old('description', $attendance->description) }}</textarea>
                                    @error('description')
                                        <p class="pt-1 text-xs text-red-600">{{ $message }}</p>
                                    @enderror
                                </td>
                            ...
                            <tr>
                                ...
                                <td>
                                    <select name="status[]" id="status" name="status"
                                        ...
                                        <option sselected value="{{ $student->status }}">-- {{ $student->status }} --
                                        </option>
                ...
                <div class="mt-5 flex justify-end">
                    <button type="submit"
                        class="dark:bg-red-600 dark:hover:bg-red-700 dark:focus:ring-red-800 w-full rounded-lg bg-red-700 px-5 py-2.5 text-center text-sm font-medium text-white hover:bg-red-800 focus:outline-none focus:ring-4 focus:ring-red-300 sm:w-auto">Simpan
                        (Check out)</button>
                </div>
            </form>
        </div>
    ...
    <script>
        function updateCheckInTime() {
            ...
            document.getElementById('check_out_time').textContent = checkInTime;
        }
